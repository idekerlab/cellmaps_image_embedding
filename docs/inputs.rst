...
If you are using historical datasets or any collection of images that was not exported as a RO-Crate, you can still run the tool by providing the imaging folders listed above plus a standalone provenance JSON file via the ``--provenance`` command-line option (see :doc:`usage`). That file should contain the same FAIRSCAPE metadata normally embedded in ``ro-crate-metadata.json`` (name, project, organization, keywords, etc.), and will be used whenever the CLI detects that the crate metadata file is missing.
